...
export const checkPermission = (user, action, resource = null) => {
    ...
    // Upload documents: only PM and Finance (Admin can view but not upload)
...
export const canAccessRoute = (user, pathname) => {
    ...
    const role = getNormalizedRole(user);
    // Manual Entry is Finance-only (no Admin access)
    if (pathname === '/finance/manual-entry') return role === ROLES.FINANCE_USER;
    if (role === ROLES.ADMIN) return true;
