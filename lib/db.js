...
export const db = {
        ...
        try {
            ...
            if (user) {
                ...
                if (role === ROLES.PROJECT_MANAGER) {
                    ...
                    // Finance users see:
                    // 1. Invoices assigned to them specifically (even if not yet in finance stage)
                    // 2. All invoices in Finance-relevant stages (Pending Review, Approved, Rejected, Paid, etc.)
                    query.$or = [
    ...
    saveInvoice: async (id, data) => {
        try {
            ...
            const updateData = {
                ...
                auditTrail: data.auditTrail || existing?.auditTrail || []
            };
    ...
    getUsersByRole: async (role) => {
        try {
            await connect();
            const users = await User.find({ role, isActive: { $ne: false } }).sort({ name: 1 });
            return users.map(u => ({
                id: u.id,
                name: u.name,
                email: u.email,
    ...
    getUsers: async (filters = {}) => {
        try {
            ...
            const docs = await User.find(query).sort({ created_at: -1 });
            return docs.map(doc => ({
                id: doc.id,
                name: doc.name,
                email: doc.email,
